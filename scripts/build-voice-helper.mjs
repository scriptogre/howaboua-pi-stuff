...
const destinationDir = resolve("src/voice/bin", `${process.platform}-${process.arch}`);
const result = spawnSync("cargo", ["build", "--release", "--locked"], { cwd: sourceRoot, stdio: "inherit", env: process.env });
